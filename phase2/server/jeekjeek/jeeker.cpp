...
bool Jeeker::check_jeek_status(string jeek_id, User* user)
{
  for(int i = 0;i < all_jeeks.size(); i++)
  {
    if(all_jeeks[i]->get_id() == jeek_id)
    {
      return all_jeeks[i]->check_user_status(user);
    }
  }
}

void Jeeker::handle_login(Interface* interface, string info)
{
  ...
  interface->status = 1;
  interface->logedin_user = loged_user;
}
...
vector<string> Jeeker::show_hashtag(string hashtag)
{
  bool is_tag_found = 0;
  ...
  for(int i = 0;i < all_jeeks.size(); i++ )
  {
    ...
    {
      is_tag_found = 1;
      result.push_back(all_jeeks[i]->show_jeeks_found());
    }
  }
  if(!is_tag_found)
...
string Jeeker::show_jeek_by_id(string info)
{
  string id;
  string extra_part;
...
vector<string> Jeeker::search(string info)
{
  string extra_part = "";
  ...
  if(searched_info[0] == '@')
  {
    ...
    User* found_user = find_user(searched_info);
     return found_user->show_user_jeeks();
  }
  else if(searched_info[0] == '#')
  {
    searched_info = searched_info.substr(1,searched_info.size());
    return show_hashtag(searched_info);
  }
  else
  {
    throw (new Bad_use_of_command);
